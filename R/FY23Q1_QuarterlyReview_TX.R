...
#
# PSNU By IM
# DATIM data as of: 2/7/2023, 23:57:01 UTC
# Genie report updated: 2/8/2023, 04:19:43 UTC
# Current period(s): 2022 Target, 2022 Q1, 2022 Q2, 2022 Q3, 2022 Q4, 2023 Target, 2023 Q1
# 
# Daily/Frozen: Daily
# Operating Unit: Tanzania,
# Funding Agency: USAID,
# Indicator: TX_CURR,TX_ML,TX_NET_NEW,TX_NEW,TX_PVLS,TX_RTT,
...
# MMD - PEDS --------------------------------------------------------------
  ...
  df_mmd_peds %>%
    ...
    expand_limits(y = 1) +
    labs(x = NULL, y = NULL,
         title = glue("USAID HAS MADE LARGE GAINS TOWARDS GETTING PEDS ON TREATMENT ON <span style='color:{golden_sand}'>+6 MONTHS</span> OF MMD"),
...
# IIT ---------------------------------------------------------------------
  ...
  df_iit %>%
    ...
    labs(x = NULL, y = NULL,
         ...
         title = glue("Sizable IIT outliers trends in Njombe in {metadata$curr_pd}") %>% toupper,
         subtitle = "",
...
# IIT - PEDS --------------------------------------------------------------
  ...
  df_iit_peds %>%
    ...
    labs(x = NULL, y = NULL,
         size = "Site TX_CURR (1 period prior)",
         title = glue("Sizable IIT outliers for peds in large treatment regions in {metadata$curr_fy_lab}") %>% toupper,
         subtitle = "",
         caption = glue("Note: IIT = TX_ML / TX_CURR_LAG1 + TX_NEW; ITT capped to 25%
                        {metadata$caption}")) +
    si_style() +
