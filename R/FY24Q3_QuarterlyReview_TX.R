...
# DATE:     2024-08-07
# UPDATED:  
# NOTE:     Adapted from FY24Q2_QuarterlyReview_TX.R


# DATIM GENIE METADATA ----------------------------------------------------

# PSNU By IM
# DATIM data as of: 8/7/2024, 00:32:45 UTC
# Genie report updated: 8/7/2024, 05:54:05 UTC
# Current period(s): 2023 Q1, 2023 Q2, 2023 Q3, 2023 Q4, 2023 Target, 2024 Q1, 2024 Q2, 2024 Q3, 2024 Target, 2025 Target
# Daily/Frozen: Daily
#
# Operating Unit: Tanzania,
# Funding Agency: USAID,
# Indicator: TX_CURR,TX_CURR_LAG2, TX_ML_IIT_six_more_mo, TX_ML_IIT_three_five_mo,TX_ML_IIT_less_three_mo,TX_NET_NEW,TX_NEW,TX_RTT,PMTCT_STAT, PMTCT_STAT_POS, PMTCT_ART
...
# NEW V NET_NEW -----------------------------------------------------------
  ...
  df_nn %>%
    ...
    labs(x = NULL, y = NULL, fill = NULL,
         title = glue("LARGELY FLAT <span style='color:{scooter}'>TX_NEW</span> TREND IN {toupper(meta$curr_fy)} BUT IMPROVING <span style='color:{scooter_light}'>NET_NEW</span> ACROSS REGIONS"),
...
# NEW V NET_NEW - PEDS ----------------------------------------------------
  ...
  df_nn_peds %>%
    ...
    labs(x = NULL, y = NULL, fill = NULL,
         title = glue("MIXED PEDS <span style='color:{golden_sand}'>TX_NEW</span> TREND IN {toupper(meta$curr_fy)} AND IMPROVING <span style='color:{golden_sand_light}'>NET_NEW</span> ACROSS REGIONS"),
...
# NEW V NET_NEW - USAID ---------------------------------------------------
  ...
  df_nn_usaid %>%
    ...
    labs(x = NULL, y = NULL, fill = NULL,
         title = glue("LARGELY FLAT <span style='color:{moody_blue}'>TX_NEW</span> TREND IN {toupper(meta$curr_fy)} WITH <span style='color:{moody_blue_light}'>NET_NEW</span> REBOUNDING"),
         caption = glue("{meta$caption}")) +
    si_style_ygrid() +
    theme(panel.spacing = unit(.5, "line"),
...
# IIT ---------------------------------------------------------------------
  ...
  df_iit %>%
    ...
    labs(x = NULL, y = NULL,
         ...
         title = glue("Mixed trend in regional IIT in {meta$curr_pd} across USAID/Tanzania") %>% toupper,
...
# IIT - PEDS --------------------------------------------------------------
  ...
  df_iit_peds %>%
    ...
    labs(x = NULL, y = NULL,
         size = "Site TX_CURR (1 period prior)",
         title = glue("Return to declining Peds IIT in {meta$curr_pd} across USAID/Tanzania") %>% toupper,
         subtitle = "Each point represents a council's IIT",
         caption = glue("Note: IIT = TX_ML (IIT)/ TX_CURR_LAG1 + TX_NEW; ITT capped to 15%
...
# IIT - USAID -------------------------------------------------------------
  ...
  df_iit_usaid %>%
    ...
    labs(x = NULL, y = NULL,
         size = "Site TX_CURR (1 period prior)",
         title = glue("Leveling off of IIT increases in {meta$curr_pd} across USAID/Tanzania") %>% toupper,
         subtitle = "Each point represents a council's IIT",
         caption = glue("Note: IIT = TX_ML (IIT) / TX_CURR_LAG1 + TX_NEW; ITT capped to 15%
                        {meta$caption}")) +
    si_style() +
...
# PMTCT -------------------------------------------------------------------
  ...
  v_p_new <- df_pmct_new %>%
    ...
    geom_jitter(aes(size = total),
                ...
                position = position_jitter(width = 0, height = 0.1), na.rm = TRUE,
                alpha = .4) +
    scale_x_continuous(label = percent_format(), position = "top", limits = c(0, NA)) +
    scale_size(guide = "none") +
    coord_cartesian(clip = "off")+
    labs(x = NULL, y = NULL,
